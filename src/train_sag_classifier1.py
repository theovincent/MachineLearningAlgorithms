...
from src.networks.sag_classifier import SAGClassifier
...
def sag_classifier_test(samples, labels, test_samples, test_labels, loss, loss_derivative, lambada, eta, add_bias=True,
    ...
    return classification_acc(sag_classifier, test_samples, test_labels)
...
def hyperparameter_opt(samples, labels, validation, valid_labels, loss, loss_derivative, add_bias):
    ...
    for idx_try in range(nb_try):
        ...
        accuracy = classification_acc(sag_classifier, validation, valid_labels)
...
def train_sag_classifier(samples, labels, validation, valid_labels, loss, loss_derivative, add_bias=True,
    ...
    if show_graph:
        ...
        # Plot the animation
        classification_visu(ortho_vects, biases, samples, labels)

    return classification_acc(sag_classifier, validation, valid_labels), lambada, eta
